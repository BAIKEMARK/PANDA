...
"""
    ...
    def generate_evaluation(self, session_id: str) -> EvaluationReport:
        ...
        # 0. 检查是否已存在评估报告
        existing_report = self.db.query(EvaluationReport).filter(
        ...
        # 5. 调用AI生成评估（如果失败则不保存任何报告）
        try:
            ai_response = search_with_ai(evaluation_prompt)
