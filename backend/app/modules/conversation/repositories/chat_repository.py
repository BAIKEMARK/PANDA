...
from backend.app.modules.conversation.schemas.chat import ChatSessionCreate, ChatMessageCreate, SessionStatus
...
class ChatRepository:
    ...
    def update_suicide_risk(self, session_id: str, has_risk: bool) -> Optional[ChatSession]:
        """更新自杀倾向状态"""
        from datetime import datetime

        db_session = self.db.query(ChatSession).filter(ChatSession.id == session_id).first()
        if db_session:
            db_session.has_suicide_risk = has_risk
            if has_risk and db_session.suicide_risk_first_detected is None:
                db_session.suicide_risk_first_detected = datetime.utcnow()
            self.db.commit()
            self.db.refresh(db_session)
        return db_session

    def alert_suicide_risk(self, session_id: str) -> Optional[ChatSession]:
        """用户点击报警按钮"""
        from datetime import datetime

        db_session = self.db.query(ChatSession).filter(ChatSession.id == session_id).first()
        if db_session:
            db_session.suicide_risk_alerted = True
            db_session.suicide_risk_alert_time = datetime.utcnow()
            self.db.commit()
            self.db.refresh(db_session)
        return db_session
