"""
恢复 app/core、app/db、app/utils 等模块的导入路径
这些模块应该使用 from core 而不是 from app.core
"""
import re
...
def fix_imports_in_file(file_path):
    ...
    # 只处理非 api 目录的文件
    if 'app\\api\\' in str(file_path) or 'app/api/' in str(file_path):
        return False

    # 恢复导入：from app.xxx -> from xxx
    patterns = [
